...
import { createResponse, createErrorResponse } from "../../../utils/apiResponse.js";
  ...
  (async () => {
    try {
      ...
      if (isNaN(pageSize) || pageSize < 1 || pageSize > 50) {
        res
          .status(400)
          .json(createErrorResponse("Invalid pageSize parameter (must be between 1 and 50)"));
        return;
      }
      ...
      const { keys: paginatedKeys, cursor: nextCursor } = await getCacheService().scanKeys(
        "package:*",
        cursor,
        pageSize
      );
      ...
      const packagesPromises = paginatedKeys.map((key: string) =>
...
router.get("/:packageName/filelist", (req: Request, res: Response) => {
  (async () => {
    try {
      ...
      res.setHeader("Content-Disposition", `attachment; filename="${packageName}-filelist.txt"`);
